...
coverage <- coverage[rownames(coverage) %in% rownames(checkm), ]
gtdb <- gtdb[rownames(gtdb) %in% rownames(checkm), ]
microtrait <- microtrait[microtrait$id %in% rownames(checkm), ]
...
# merge
microtrait_tax2 <- merge(microtrait, gtdb2, by.x="id", by.y="row.names")
microtrait_merged2 <- merge(microtrait_tax2, coverage_mean, by.x="id", by.y="label")

# calculate weighted genus counts
traits <- microtrait_merged2 %>% 
  mutate(across(all_of(trait_list), ~ . * mean_rel_abund)) %>% # count weighted by mean relative abundance
  group_by(phylum, genus) %>% 
  summarise(across(all_of(trait_list), sum),) # sum trait presence by genus

# prepare table for plotting
traits2 <- traits %>% 
  column_to_rownames(., var="genus") %>% 
  select(where(~ !is.numeric(.) || sum(.) != 0)) %>% # remove traits that are all zero
...
traits_melt <- traits2 %>% 
  ...
  separate(variable, into = c("level1", "level2", "level3","level4","level5", "level6"), # separate traits into their levels
           ...
           extra="merge",
           fill="right") #%>% 
  # mutate(genus=factor(genus, levels=sort(unique(genus), decreasing=T))) %>%  # reverse genus order for plotting
  # arrange(genus)
...
traits_melt_sub <- traits_melt2[traits_melt2$genus %in% rownames(coverage_mean_genera), ]

genus_order <- traits_melt_sub %>%
  distinct(genus, phylum) %>%
  arrange(phylum, genus) %>%
  pull(genus)

traits_melt_sub$genus <- factor(traits_melt_sub$genus, levels = genus_order)

# plot
level_4_plots <- lapply(ordered_groups, function(group) {
  group_data <- traits_melt_sub %>% filter(level2 == group)
  
  group_data <- traits_melt_sub %>%
    filter(level2 == group) %>%
    mutate(genus = factor(genus, levels = rev(genus_order))) 
  
  ggplot(group_data, aes(x = level4, y = genus, fill = value)) +
    geom_tile() +
    scale_fill_gradientn(colors = brewer.pal(9, "Blues"),
                         limits = c(0,0.05),
                         name = "Weighted count",
    ...
    theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.4),
          ...
          panel.grid.minor = element_blank(),
          legend.position="bottom",
          legend.text = element_text(angle = 45, size = 5, hjust = 1),
          strip.background = element_blank(),
          strip.text = element_text(size = 6, colour = "black"),
          plot.title = element_text(size=6, colour="black", hjust=0.5),
          axis.title = element_blank(),
...
level_5_plots <- lapply(ordered_groups, function(group) {
  ...
  ggplot(group_data, aes(x = level5, y = genus, fill = value)) +
    geom_tile() +
    scale_fill_gradientn(colors = brewer.pal(9, "Blues"),
                         limits = c(0,0.05),
                         name = "Weighted count",
...
ggsave("output/microtrait_genus_combined2.png", patchwork, width = 18, height = 8, units = "in", dpi = 600)
ggsave("output/microtrait_genus_combined2.pdf", patchwork, width = 220, height = 140, units = "mm", dpi = 600)
